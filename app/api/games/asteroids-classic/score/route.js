// app/api/games/asteroids-classic/score/route.js  ->  POST /api/games/asteroids-classic/score
//
...
export async function POST(request) {
  ...
  const allowed = await checkRateLimit("asteroids-classic-score", ip, 20, "1 m");
  if (!allowed) {
  ...
  const playSession = await redis.getdel(`asteroids-classic:session:${token}`);
  ...
  if (account) {
    ...
    const beforeRank = await redis.zrevrank("asteroids-classic:leaderboard", id);
